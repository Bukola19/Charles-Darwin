...
fastqc "${name}_R1.fastq.gz" "${name}_R2.fastq.gz" -o "QC-reports"
...
# Variant calling using bcftools
  ...
  bcftools mpileup -Ou -f "$ref_name" Mapping/"$name".sorted.bam | bcftools call -mv -Ov -mv > Variant-calling/"$name".vcf
done
